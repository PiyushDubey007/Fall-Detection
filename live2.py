...
import winsound  # Windows audio alarm
...
class Config:
    # Model Settings
    MODEL_PATH: str = "huggingface"  # Use HuggingFace model by default
    ...
    NETWORK_CHECK_INTERVAL: float = 5.0  # seconds
    LATENCY_HISTORY_SIZE: int = 100
    PACKET_LOSS_THRESHOLD: float = 0.05  # 5%
    JITTER_THRESHOLD: float = 50  # ms
...
class AsyncLogger:
    """Thread-safe async logger with rotation and structured output"""
...
def _play_alarm():
    """Play beeping alarm in a background thread (non-blocking)."""
    for _ in range(5):
...
class AlertManager:
    ...
    def check_alert(self, detection_type: str, confidence: float) -> Tuple[bool, str]:
        ...
        if self.consecutive_detections[detection_type] >= self.config.FALL_CONFIRMATION_FRAMES:
            ...
            self.fall_history.append({"time": datetime.now().isoformat(), "type": detection_type, "confidence": confidence})
...
# ═══════════════════════════════════════════════════════════════════════════════
# DASHBOARD STATE  (shared between detection thread and FastAPI)
# ═══════════════════════════════════════════════════════════════════════════════
...
class FallDetectionSystem:
    """Production-ready fall detection system"""
    ...
    def connect_camera(self) -> bool:
        try:
            ...
            self.logger.log_system("camera_connection_failed", {"error": str(e), "attempt": self.reconnect_attempts}, "ERROR")
            return False
    ...
    def process_frame(self, frame: np.ndarray) -> np.ndarray:
                ...
                if cls_name == "fallen":
                    ...
                    if should_alert:
                        self.alert_manager.trigger_alert(
                            "FALL_DETECTED", {"confidence": conf_val, "bbox": [x1, y1, x2, y2], "reason": reason}, frame
                        )
                        if self.config.SAVE_ALERT_FRAMES:
                            clip_path = self.frame_buffer.save_event_clip(datetime.now(), "fall", self.config.ALERT_DIR)
                            if clip_path:
                    ...
                    if should_alert:
                        self.alert_manager.trigger_alert(
                            "FALLING_DETECTED", {"confidence": conf_val, "bbox": [x1, y1, x2, y2], "reason": reason}, frame
                        )
                else:
                    box_color = (0, 200, 0)
        ...
        # Push JPEG for dashboard stream
    ...
    def run(self):
        ...
        while self.running:
            ...
            # Small sleep to prevent CPU spin — no OpenCV window needed
            time.sleep(0.001)
...
if __name__ == "__main__":
    ...
    print("  ✅ Audio alarm on fall detection")
