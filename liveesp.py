...
Features: Advanced logging, 5G metrics, network resilience, Web Dashboard, ESP32 Hardware Alarm
...
class Config:
    ...
    # Video Source
    IP_CAM_URL: str = "http://10.71.243.26:8080/video"
    FRAME_WIDTH: int = 860
    ...
    ESP32_PORT: str = "COM4"      # ← change to your ESP32's COM port
    ESP32_BAUD: int = 9600
...
class NetworkMonitor:
    ...
    def get_network_quality(self) -> Tuple[str, dict]:
        ...
        metrics = {
            "avg_latency_ms": round(avg_latency, 2), "avg_jitter_ms": round(avg_jitter, 2),
            "packet_loss_rate": round(packet_loss_rate, 4), "quality_grade": quality,
...
class CircularFrameBuffer:
    ...
    def save_event_clip(self, event_time: datetime, event_type: str, alert_dir: str):
        ...
        frames_to_save = [
            f for f, ts in zip(self.buffer, self.timestamps)
            if -self.config.PRE_EVENT_BUFFER_SEC <= (ts - event_time).total_seconds() <= self.config.POST_EVENT_BUFFER_SEC
        ]
...
class ESP32Controller:
    """Manages serial communication with the ESP32 hardware alarm."""
    ...
    def _connect(self):
        if not self.config.ESP32_ENABLED:
            return
        try:
            ...
            time.sleep(3)  # Wait for ESP32 to reset after serial open
            self.logger.log_system("esp32_connected", {"port": self.config.ESP32_PORT})
            print(f"✅ ESP32 connected on {self.config.ESP32_PORT}")
        except serial.SerialException as e:
            self.serial = None
            self.logger.log_system("esp32_connection_failed", {"error": str(e), "port": self.config.ESP32_PORT}, "WARNING")
            print(f"⚠️  ESP32 not connected ({self.config.ESP32_PORT}): {e}")
            print("   Hardware alarm disabled. PC alarm still active.")

    def trigger_hardware_alarm(self):
        """Send ALERT command to ESP32 (non-blocking)."""
        if self.serial is None:
            return
        def _send():
            with self._lock:
                try:
                    self.serial.write(b"ALERT\n")
                    self.serial.flush()
                except serial.SerialException as e:
                    self.logger.log_system("esp32_send_failed", {"error": str(e)}, "WARNING")
...
class AlertManager:
    def __init__(self, config: Config, logger: AsyncLogger, esp32: "ESP32Controller"):
        self.config  = config
        self.logger  = logger
        self.esp32   = esp32
    ...
    def trigger_alert(self, alert_type: str, details: dict, frame: Optional[np.ndarray] = None):
        ...
        # ESP32 hardware alarm only (no winsound)
        self.esp32.trigger_hardware_alarm()
...
class FallDetectionSystem:
    def __init__(self, config: Config = None, dashboard_state: DashboardState = None):
        ...
        self.esp32           = ESP32Controller(self.config, self.logger)
        self.alert_manager   = AlertManager(self.config, self.logger, self.esp32)
    ...
    def initialize_model(self) -> bool:
        try:
            ...
            self.logger.log_system("model_loaded", {
                "model": str(self.config.MODEL_PATH), "task": self.model.task, "names": self.model.names
            })
            return True
    ...
    def connect_camera(self) -> bool:
        try:
            ...
            self.logger.log_system("camera_connection_failed",
    ...
    def process_frame(self, frame: np.ndarray) -> np.ndarray:
        ...
        results = self.model(
            ...
            verbose=False, device=0 if self.config.ENABLE_GPU else 'cpu'
        )[0]
        ...
        if boxes is not None and len(boxes):
            for box in boxes:
                ...
                if cls_name == "fallen":
                    ...
                    if should_alert:
                        ...
                        if self.config.SAVE_ALERT_FRAMES:
                            clip = self.frame_buffer.save_event_clip(datetime.now(), "fall", self.config.ALERT_DIR)
                            if clip:
                                self.logger.log_system("alert_clip_saved", {"path": clip})
                ...
                detections.append({"class": cls_name, "confidence": round(conf_val, 3), "bbox": [x1, y1, x2, y2]})
    ...
    def _draw_overlay(self, frame, fps, latency, detections):
        h, w = frame.shape[:2]
        ...
        quality, _ = self.net_monitor.get_network_quality()
        quality_colors = {"EXCELLENT":(0,255,0),"GOOD":(0,200,0),"FAIR":(0,255,255),"POOR":(0,0,255),"UNKNOWN":(128,128,128)}
...
<script>
  ...
  async function dismissAlarm(){
    ...
    alarmActive=false;document.getElementById('alarm-overlay').classList.remove('active');
  }
...
if __name__ == "__main__":
    ...
    print("  ✅ PC audio alarm (winsound)")
    print("  ✅ ESP32 hardware alarm (buzzer + LED)")
    print("  ✅ Web dashboard at http://localhost:8000")
    print("=" * 70)
    print("\nPress Ctrl+C to quit\n")

    # Print available COM ports to help identify ESP32
    ports = [p.device for p in serial.tools.list_ports.comports()]
    print(f"Available COM ports: {ports}")
    print(f"Using ESP32 port: COM5  (change ESP32_PORT in Config if needed)\n")
